...
It combines **LangGraph** (workflow orchestration), **LangChain** (SQL agent tooling), **Google Gemini** (via Vertex AI), and a **PostgreSQL** customer-support database with 50+ products, multi-tier support agents, tickets, and resolutions. The workflow is exposed over HTTP via a **FastAPI** service, including support for resuming human-review escalations across separate requests.
...
- REST API (FastAPI) for submitting complaints, polling status, and resolving human-review escalations
...
├── api/
│   ├── main.py               # FastAPI app: /complaints, /complaints/{id}, /complaints/{id}/review
...
The workflow is also exposed over HTTP via FastAPI (`api/main.py`):
...
Each complaint runs as a LangGraph **thread** (`thread_id`), so escalated complaints can be paused for human review and resumed later from a separate request.

| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/health` | Health check |
| `POST` | `/complaints` | Submit a new complaint and run the workflow |
| `GET` | `/complaints/{thread_id}` | Check a complaint thread's current status |
| `POST` | `/complaints/{thread_id}/review` | Resume a thread awaiting human review |
...
curl -X POST http://localhost:8000/complaints \
  ...
  -d '{
    "complaint_text": "My laptop screen has a dead pixel",
    "customer_email": "jane@example.com"
  }'
```

A response either resolves immediately (`status: "completed"`) with a `response`, `category`, `priority`, and `ticket_ref`, or — for complaints that match escalation language (legal threats, repeated complaints, fraud, etc.) — returns `status: "pending_human_review"` along with a `review_payload` and the `thread_id` needed to resume it.

**Resume an escalated thread:**

```bash
curl -X POST http://localhost:8000/complaints/<thread_id>/review \
  -H "Content-Type: application/json" \
  -d '{"decision": "approve", "comments": "Confirmed, escalate to senior agent."}'
```

> **Note:** the API uses an in-memory `MemorySaver` checkpointer to persist thread state between the initial request and the review request. This is fine for local development, but pending-review threads are lost on server restart — swap in a persistent checkpointer (e.g. `langgraph.checkpoint.postgres.PostgresSaver` against the same `customer_care` database) before relying on this in production.
...
- `human_review_node` uses LangGraph's `interrupt()`, so the graph pauses and persists state until resumed with a human `decision` and `comments`. This requires a checkpointer: `langgraph dev` provides its own, and the FastAPI app (`api/main.py`) compiles the graph with a `MemorySaver` for the same purpose.
